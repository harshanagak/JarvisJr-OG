    ...
    def run_command(self, event=None):
        ...
        if True:
            if command.strip():
                ...
                prompt=f"Preveous input: \n {self.previn} \n Previous Output: \n {self.prevre}. \n Now Considering all the previous data, Write a windows command prompt command to '{command}'. "
                f"Please describe the action you want to perform in a Windows environment "
                f"using simple English. For instance, 'Create a new text file', 'List all files in a directory', "
                f"or 'Delete a specific file'.\nBased on your description, please provide the Windows command in the format decription:, command:(in this section just give the executable comd and ntg else and finsish)",
                ...
                generated_command = openai.Completion.create(
                engine="text-davinci-003",  # Use the GPT-3.5 engine
                prompt=f" '{generated_command}' "
                f"get me windows CLI Executable command from response,remember only command,without any inverted commas",
                max_tokens=30,  # Adjust the maximum number of tokens as needed
                api_key=api_key,
            )
                # print(response)
                # Extract the generated command from the response
                generated_command = generated_command.choices[0].text.strip()
                print(generated_command)
